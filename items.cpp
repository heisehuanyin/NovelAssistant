    ...
    descBlock(new QTextEdit)
{
    ...
    this->connect(smodel, &QItemSelectionModel::selectionChanged,
                  this,   &Items::slot_responseItemSelection);
...
QList<QVariant> Items::getSelectedItems()
{
    auto instance(new Items);
    instance->tableModel->changeCheckable(true);
    instance->exec();
    return instance->tableModel->selectedRecordIDs();
}

void Items::slot_queryProps(const QString &text)
{
...
void Items::slot_responseItemSelection(const QItemSelection &, const QItemSelection &)
{
    auto index = this->table->currentIndex();
    if(!index.isValid())
        return;
